...
class Square:
    ...
    def __str__(self):
        area_coordinate = []
        if self.__size == 0:
            area_coordinate.append('\n')
        else:
            area_coordinate.append('\n' * self.__position[1])
            for x in range(1, self.__size + 1):
                area_coordinate.append(' ' * self.__position[0])
                area_coordinate.append('#' * self.__size + '\n')
        list_repr = ''.join(area_coordinate)
        return list_repr.rstrip()  # Removes the extra newline added by `join()`
